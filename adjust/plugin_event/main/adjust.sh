...
sed -i "/^sleep.*/a kubectl set image deployment/3scale-kourier-gateway kourier-gateway=docker-na.artifactory.swg-devops.com/sys-linux-power-team-ftp3distro-docker-images-docker-local/knative/maistra/envoy:v2.4 -n kourier-system" test/e2e-tests.sh
